...
TSMS_INLINE TSMS_SMNP __internal_tsms_create_string_node(pString key, void * value) {
	TSMS_SMNP node = malloc(sizeof(struct TSMS_STRING_MAP_NODE));
	node->key = key;
	node->value = value;
	return node;
}
...
TSMS_SMP TSMS_STRING_MAP_create(TSMS_SIZE diffusion) {
	TSMS_SMP map = malloc(sizeof(struct TSMS_STRING_MAP));
	if (map == TSMS_NULL) {
		tString temp = TSMS_STRING_temp("malloc failed for map");
		TSMS_ERR_report(TSMS_ERR_MALLOC_FAILED, &temp);
		return TSMS_NULL;
	}
	map->base = malloc(sizeof( TSMS_SMNP) * diffusion);
	map->diffusion = diffusion;
	map->size = 0;
	return map;
}
...
TSMS_RESULT TSMS_STRING_MAP_put(TSMS_SMP map, pString key, void * value) {
	if (map == TSMS_NULL)
		return TSMS_ERROR;
	TSMS_POS offset = TSMS_STRING_hash(key);
	TSMS_SMNP cur = map->base[offset];
	// first if not exist
	if (cur == TSMS_NULL)
		map->base[offset] = __internal_tsms_create_string_node(key, value);
	else {
		// if existed, judge it!
		if (cur->key == key) {
			if (cur->value == value)
		...
		if (cur->next == TSMS_NULL)
			cur->next = __internal_tsms_create_string_node(key, value);
	}
	map->size++;
	return TSMS_SUCCESS;
}
...
TSMS_SMI TSMS_STRING_MAP_iterator(TSMS_SMP map) {
	if (map == TSMS_NULL)
		return TSMS_EMPTY_STRING_MAP_ITERATOR;
	TSMS_SMI iter = {map, 0, map->base[0]};
	return iter;
}

bool TSMS_STRING_MAP_hasNext(TSMS_SMIP iter) {
	if (iter == TSMS_NULL || iter == &TSMS_EMPTY_STRING_MAP_ITERATOR)
		return false;
	if (iter->next == TSMS_NULL) {
		TSMS_POS i = iter->current + 1;
		for (; i < iter->map->diffusion; i++)
...
TSMS_SME TSMS_STRING_MAP_next(TSMS_SMIP iter) {
	if (iter == TSMS_NULL || iter == &TSMS_EMPTY_STRING_MAP_ITERATOR)
		return TSMS_EMPTY_STRING_MAP_ENTRY;
	if (iter->next == TSMS_NULL) {
		TSMS_POS i = iter->current + 1;
		for (; i < iter->map->diffusion; i++)
			if (iter->map->base[i] != TSMS_NULL) {
