...
struct AD7606_Handler * AD7606_initSerialHardware(GPIO_TypeDef* byteSelect,uint16_t byteSelectPin,GPIO_TypeDef * db15, uint16_t db15Pin,GPIO_TypeDef* standBy,uint16_t standByPin,GPIO_TypeDef* range,uint16_t rangePin,GPIO_TypeDef* convstA,uint16_t convstAPin,GPIO_TypeDef* convstB,uint16_t convstBPin,GPIO_TypeDef* os0,uint16_t os0Pin,GPIO_TypeDef* os1,uint16_t os1Pin,GPIO_TypeDef* os2,uint16_t os2Pin,GPIO_TypeDef* referenceSelect,uint16_t referenceSelectPin,
	...
	handler->handler = TSMS_DRIVER_createSPIHandler(TSMS_SPI_createSoftwareHandler(TSMS_GPIO_createHandler(cs, csPin), TSMS_GPIO_createHandler(sclk, sclkPin),
	                                              TSMS_GPIO_createHandler(doutA, doutAPin),TSMS_NULL_GHP, TSMS_SPI_MODE_3, TSMS_GPIO_LOW, TSMS_TRANSFER_MSB),
	                                                TSMS_REG_createList(0));
	handler->overSample = TSMS_CUSTOM_createParallelHandler(TSMS_NULL, TSMS_TRANSFER_LSB, 3, TSMS_GPIO_createHandler(os0, os0Pin), TSMS_GPIO_createHandler(os1, os1Pin), TSMS_GPIO_createHandler(os2, os2Pin));
	...
	handler->mode = TSMS_CUSTOM_createParallelHandler(TSMS_NULL, TSMS_TRANSFER_LSB, 2, TSMS_GPIO_createHandler(standBy, standByPin),
...
float * AD7606_readData(struct AD7606_Handler* handler) {
	if (handler->handler->type == TSMS_DRIVER_SPI) {
		...
		for (int i = 0; i < 8; i++)
			AD7606_FLOAT_DATA[i] = ((int16_t)AD7606_DATA[i]) * handler->voltage * 2.5 / handler->referenceVoltage / 32768;
	}
